...
def refresh_stats(open_id):
    ...
    # Stats fields require TikTok to grant the corresponding user-info permission.
    fields = "open_id,display_name,avatar_url,username,follower_count,following_count,likes_count,video_count"
    try:
        r = requests.get(
            USER_URL,
        ...
        if not r.ok or err.get("code") not in ("ok", "", None):
            flash("Statistik belum dapat dibaca dari TikTok. Pastikan scope user.info.stats/profile disetujui. " +
        ...
        c.execute("""UPDATE accounts SET
            ...
            video_count=?, following_count=?, follower_count=?, likes_count=?,
            stats_updated_at=?
            WHERE open_id=?""", (
        ...
        c.commit()
        c.close()
        flash("Statistik akun diperbarui.", "success")
    except Exception as e:
