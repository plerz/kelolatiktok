...
def dashboard():
    ...
    accounts = c.execute("""
        SELECT open_id,display_name,avatar_url,scope,connected_at,
               ...
               likes_count,view_count,stats_updated_at
...
def account_details(open_id):
    ...
    c = db()
    c.execute("UPDATE accounts SET email=?, username=? WHERE open_id=?",
              (email, username, open_id))
    c.commit()
    c.close()
...
def create_photo(open_id):
    ...
    api_error = info.get("error", {})
    if request.method == "GET":
        return render_template("create_photo.html", account=account, creator=creator, api_error=api_error)
    ...
    if uploaded and any(f and f.filename for f in uploaded):
        ...
        for idx, f in enumerate(uploaded, start=1):
            ...
            if ext not in {".jpg", ".jpeg", ".png", ".webp"}:
                ...
                return redirect(url_for("create_photo", open_id=open_id, mode=request.args.get("mode", "carousel")))
            safe_name = f"{idx:02d}{ext}"
            f.save(os.path.join(batch_dir, safe_name))
            photo_urls.append(request.url_root.rstrip("/") + url_for("uploaded_photo", batch=batch, filename=safe_name))
    if not consent:
        flash("Centang persetujuan sebelum mengirim foto ke TikTok.", "error")
        return redirect(url_for("create_photo", open_id=open_id))
    if not photo_urls:
        flash("Pilih minimal satu file foto atau masukkan URL foto publik.", "error")
        return redirect(url_for("create_photo", open_id=open_id))
    if len(photo_urls) > 35:
        flash("Terlalu banyak foto. Kurangi jumlah foto carousel.", "error")
        return redirect(url_for("create_photo", open_id=open_id))
    options = creator.get("privacy_level_options", [])
    ...
    payload = {
        "post_info": {
            "title": caption[:90],
            "description": caption,
            "disable_comment": bool(creator.get("comment_disabled")) or request.form.get("allow_comment") != "yes",
            "privacy_level": privacy,
    ...
    try:
        ...
        flash("Photo Post gagal: " + str(e), "error")
        return redirect(url_for("create_photo", open_id=open_id))
    err = result.get("error", {})
    publish_id = result.get("data", {}).get("publish_id", "")
    if not pr.ok or err.get("code") not in ("ok", "", None) or not publish_id:
        flash("TikTok menolak Photo/Carousel Post: " + json.dumps(result, ensure_ascii=False), "error")
        return redirect(url_for("create_photo", open_id=open_id))
    c=db()
    if c.is_pg:
        c.execute("""INSERT INTO publish_jobs(publish_id,open_id,caption,privacy_level,status,created_at,last_response)
                     VALUES(?,?,?,?,?,?,?) ON CONFLICT (publish_id) DO UPDATE SET status=EXCLUDED.status,last_response=EXCLUDED.last_response""",
                  (publish_id,open_id,caption,privacy,"PROCESSING",datetime.utcnow().isoformat(),json.dumps(result,ensure_ascii=False)))
    else:
        c.execute("""INSERT OR REPLACE INTO publish_jobs(publish_id,open_id,caption,privacy_level,status,created_at,last_response)
                     VALUES(?,?,?,?,?,?,?)""",
                  (publish_id,open_id,caption,privacy,"PROCESSING",datetime.utcnow().isoformat(),json.dumps(result,ensure_ascii=False)))
    c.commit(); c.close()
    flash("Foto/carousel sudah dikirim ke TikTok dan sedang diproses.", "success")
    return redirect(url_for("publish_status", open_id=open_id, publish_id=publish_id))
