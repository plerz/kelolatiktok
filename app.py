...
def _bulk_publish_video(account, file_path, original_name, caption, requested_privacy):
    """One compliant Direct Post attempt for one authorized account."""
    r, info = get_creator_info(account)
    creator = info.get("data", {})
    err = info.get("error", {})
    if r is None or not r.ok or err.get("code") not in ("ok", "", None):
        return False, "creator_info: " + json.dumps(info, ensure_ascii=False)[:800], None
    options = creator.get("privacy_level_options", [])
    privacy = requested_privacy if requested_privacy in options else ("SELF_ONLY" if "SELF_ONLY" in options else None)
    if not privacy:
        return False, "Tidak ada privacy_level yang tersedia untuk akun ini.", None
    try:
        ...
        up = requests.put(data["upload_url"], headers={
            "Content-Type":mime,"Content-Length":str(size),"Content-Range":f"bytes 0-{size-1}/{size}"
        }, data=video_bytes, timeout=240)
        if not up.ok:
            return False, f"Upload HTTP {up.status_code}: {up.text[:800]}", data.get("publish_id")
...
def _run_bulk_queue():
    ...
    try:
        while True:
            c = db()
            job = c.execute("""SELECT j.*, b.file_path,b.original_name,b.caption,b.privacy_level,b.category
                               FROM bulk_jobs j JOIN bulk_batches b ON b.id=j.batch_id
                               WHERE j.status='QUEUED' ORDER BY j.id ASC LIMIT 1""").fetchone()
            c.close()
            if not job: break
            account = get_account(job["open_id"])
            c = db()
            c.execute("UPDATE bulk_jobs SET status='PROCESSING',attempts=attempts+1,started_at=? WHERE id=?",
                      (datetime.utcnow().isoformat(), job["id"]))
            c.execute("UPDATE bulk_batches SET status='PROCESSING',started_at=COALESCE(started_at,?) WHERE id=?",
                      (datetime.utcnow().isoformat(), job["batch_id"]))
            c.commit(); c.close()
            if not account:
                ok, msg, publish_id = False, "Akun sudah tidak tersedia.", None
            else:
                ok, msg, publish_id = _bulk_publish_video(account, job["file_path"], job["original_name"], job["caption"] or "", job["privacy_level"])
            c = db()
            status = "SENT" if ok else "FAILED"
            c.execute("UPDATE bulk_jobs SET status=?,publish_id=?,last_error=?,finished_at=? WHERE id=?",
                      (status,publish_id,None if ok else msg,datetime.utcnow().isoformat(),job["id"]))
            if ok:
                c.execute("UPDATE bulk_batches SET success_count=success_count+1 WHERE id=?", (job["batch_id"],))
            else:
                c.execute("UPDATE bulk_batches SET failed_count=failed_count+1 WHERE id=?", (job["batch_id"],))
            remain = c.execute("SELECT COUNT(*) AS n FROM bulk_jobs WHERE batch_id=? AND status IN ('QUEUED','PROCESSING')", (job["batch_id"],)).fetchone()["n"]
            if int(remain) == 0:
                c.execute("UPDATE bulk_batches SET status='DONE',finished_at=? WHERE id=?", (datetime.utcnow().isoformat(),job["batch_id"]))
            c.commit(); c.close()
            # Conservative queue pacing; do not retry TikTok spam/banned responses automatically.
            time.sleep(BULK_START_INTERVAL_SECONDS)
    finally:
...
def bulk_upload():
    c = db()
    categories = c.execute("""SELECT category, COUNT(*) AS total FROM accounts
                              WHERE category IS NOT NULL AND TRIM(category)<>''
                              GROUP BY category ORDER BY category""").fetchall()
    batches = c.execute("SELECT * FROM bulk_batches ORDER BY id DESC LIMIT 20").fetchall()
    c.close()
    if request.method == "GET":
        return render_template("bulk_upload.html", categories=categories, batches=batches, interval=BULK_START_INTERVAL_SECONDS)
    category = request.form.get("category", "").strip()
    caption = request.form.get("caption", "").strip()
    privacy = request.form.get("privacy_level", "SELF_ONLY").strip()
    consent = request.form.get("consent") == "yes"
    video = request.files.get("video")
    if not consent:
        flash("Centang persetujuan bulk posting.", "error"); return redirect(url_for("bulk_upload"))
    if not category:
        flash("Pilih kategori akun.", "error"); return redirect(url_for("bulk_upload"))
    if not video or not video.filename:
        flash("Pilih satu file video.", "error"); return redirect(url_for("bulk_upload"))
    ext = os.path.splitext(video.filename.lower())[1]
    if ext not in {".mp4",".mov",".webm"}:
        flash("Format video harus MP4, MOV, atau WEBM.", "error"); return redirect(url_for("bulk_upload"))
    data = video.read()
    if not data or len(data) > MAX_UPLOAD_BYTES:
        flash("Video kosong atau melebihi 64 MB.", "error"); return redirect(url_for("bulk_upload"))
    c = db()
    accounts = c.execute("SELECT open_id FROM accounts WHERE LOWER(TRIM(category))=LOWER(TRIM(?)) ORDER BY connected_at", (category,)).fetchall()
    if not accounts:
        c.close(); flash("Tidak ada akun dalam kategori tersebut.", "error"); return redirect(url_for("bulk_upload"))
    folder = os.path.join(PHOTO_UPLOAD_DIR, "bulk_files"); os.makedirs(folder, exist_ok=True)
    stored = os.path.join(folder, "bulk_" + secrets.token_urlsafe(12).replace("-","").replace("_","") + ext)
    with open(stored,"wb") as f: f.write(data)
    now = datetime.utcnow().isoformat()
    if c.is_pg:
        cur = c.execute("""INSERT INTO bulk_batches(category,post_type,caption,privacy_level,file_path,original_name,total_accounts,queued_count,status,created_at)
                           VALUES(?,?,?,?,?,?,?,?,?,?) RETURNING id""",
                        (category,"video",caption,privacy,stored,video.filename,len(accounts),len(accounts),"QUEUED",now))
    ...
    for a in accounts:
        c.execute("INSERT INTO bulk_jobs(batch_id,open_id,status,attempts,created_at) VALUES(?,?,?,?,?)", (batch_id,a["open_id"],"QUEUED",0,now))
    c.commit(); c.close()
    _ensure_bulk_worker()
    flash(f"Bulk upload masuk antrean untuk {len(accounts)} akun kategori {category}.", "success")
    return redirect(url_for("bulk_status", batch_id=batch_id))

@app.get("/bulk-upload/<int:batch_id>")
def bulk_status(batch_id):
    _ensure_bulk_worker()
    c = db()
    batch = c.execute("SELECT * FROM bulk_batches WHERE id=?", (batch_id,)).fetchone()
    jobs = c.execute("""SELECT j.*,a.display_name,a.username FROM bulk_jobs j
                        LEFT JOIN accounts a ON a.open_id=j.open_id WHERE j.batch_id=? ORDER BY j.id""", (batch_id,)).fetchall()
    c.close()
    if not batch: return "Batch tidak ditemukan", 404
    return render_template("bulk_status.html", batch=batch, jobs=jobs, interval=BULK_START_INTERVAL_SECONDS)
