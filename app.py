...
def init_db():
    ...
    for name, typ in {"interval_seconds":"INTEGER DEFAULT 12", "paused":"INTEGER DEFAULT 0"}.items():
...
def _run_bulk_queue():
    ...
    try:
        while True:
            c=db()
            job=c.execute("""SELECT j.*,b.file_path,b.original_name,b.caption,b.privacy_level,b.category,b.post_type,b.interval_seconds,b.paused
                              FROM bulk_jobs j JOIN bulk_batches b ON b.id=j.batch_id
                              WHERE j.status='QUEUED' AND COALESCE(b.paused,0)=0
                              ORDER BY j.id ASC LIMIT 1""").fetchone()
            c.close()
            if not job: break
...
def bulk_upload():
    c=db(); categories=c.execute("""SELECT category,COUNT(*) AS total FROM accounts WHERE category IS NOT NULL AND TRIM(category)<>'' GROUP BY category ORDER BY category""").fetchall(); batches=c.execute("SELECT * FROM bulk_batches ORDER BY id DESC LIMIT 20").fetchall(); c.close()
    if request.method=="GET": return render_template("bulk_upload.html",categories=categories,batches=batches,interval=BULK_START_INTERVAL_SECONDS)
    ...
    if c.is_pg:
        cur=c.execute("""INSERT INTO bulk_batches(category,post_type,caption,privacy_level,file_path,original_name,total_accounts,queued_count,status,created_at,interval_seconds,paused) VALUES(?,?,?,?,?,?,?,?,?,?,?,?) RETURNING id""",(category,post_type,caption,privacy,stored_value,original_name,len(accounts),len(accounts),"QUEUED",now,interval,0)); batch_id=cur.fetchone()["id"]
    else:
        cur=c.execute("""INSERT INTO bulk_batches(category,post_type,caption,privacy_level,file_path,original_name,total_accounts,queued_count,status,created_at,interval_seconds,paused) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",(category,post_type,caption,privacy,stored_value,original_name,len(accounts),len(accounts),"QUEUED",now,interval,0)); batch_id=cur.lastrowid
